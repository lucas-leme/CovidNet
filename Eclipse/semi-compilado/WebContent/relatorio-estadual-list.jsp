...
<body>
    ...
    <div align="center">
    	...
    	<h1>Fluxo</h1>
			...
			<c:forEach varStatus="loop" items="${alocs}">
                <tr>
                    <td><c:out value="${pedidos[loop.index].dataPedido}" /></td>
                    <td><c:out value="${pedidos[loop.index].pedidos}" /></td>
                    <td><c:out value="${alocs[loop.index].alocacoes}" /></td>
                </tr>
			</c:forEach>
			</table>
		...
		<h1>Ocupação</h1>
			...
			<c:forEach varStatus="loop" items="${vagas}">
                <tr>
                	<td><c:out value="${vagas[loop.index].data}" /></td>
                    <td><c:out value="${vagas[loop.index].vagasOcupadas}" /></td>
                    <td><c:out value="${ocupacao[loop.index]}%" /></td>
                </tr>
			</c:forEach>
			</table>
